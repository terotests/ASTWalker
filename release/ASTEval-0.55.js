  ...
  var ASTEval_prototype = function ASTEval_prototype() {
    ...
    (function (_myTrait_) {
      ...
      _myTrait_.findAndSetLet = function (name, ctx, value) {
        // does the context have this variable?
        if (ctx.letVars && typeof ctx.letVars[name] != "undefined") {
          ctx.letVars[name] = value;
          return true;
        } else {
          if (ctx.parentCtx) {
      ...
      _myTrait_.findConstVar = function (name, ctx) {
        if (ctx.constVars && typeof ctx.constVars[name] != "undefined") {
      ...
      _myTrait_.ForStatement = function (node, ctx) {
        ...
        // for statement has also it's own context
        ...
        var max_cnt = 10000; // <-- maximum loop count, temporary setting...

        while (max_cnt > 0) {
          if (node.test) {
            this.walk(node.test, myCtx);
            if (!node.text.eval_res) {
              break;
            }
          } else {
